...
# Assign each server to the BGP group.
# NOTE: BGP daemon config (BIRD2/FRR) is handled by Ansible, not Terraform.
# Terraform assigns the BGP group at the API level only.
resource "netactuate_bgp_sessions" "node" {
  for_each = netactuate_server.node

  mbpkgid  = tonumber(each.value.id)
